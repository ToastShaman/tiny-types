...
public final class Tries {
    ...
    static <T1, T2, U> Try<U> flatZip(Try<T1> t1, Try<T2> t2, Function2<T1, T2, Try<U>> fn) {
        return t1.flatMap(a -> t2.flatMap(b -> fn.apply(a, b)));
    }

    static <T1, T2, T3, U> Try<U> flatZip(Try<T1> t1, Try<T2> t2, Try<T3> t3, Function3<T1, T2, T3, Try<U>> fn) {
        return t1.flatMap(a -> t2.flatMap(b -> t3.flatMap(c -> fn.apply(a, b, c))));
    }

    static <T1, T2, T3, T4, U> Try<U> flatZip(
    ...
    static <T> Tuple2<List<T>, List<Throwable>> partition(Iterable<Try<T>> values) {
    ...
    static <T> List<T> anyValues(Iterable<Try<T>> values) {
    ...
    static <T> Try<List<T>> allValues(Iterable<Try<T>> values) {
    ...
    static <T, T1> Try<T1> foldResult(Iterable<T> values, Try<T1> initial, Function2<T1, T, Try<T1>> operation) {
    ...
    static <T, T1> Try<List<T1>> mapAllValues(Iterable<T> values, Function<T, Try<T1>> f) {
